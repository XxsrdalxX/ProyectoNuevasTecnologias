    ...
    return `
        <div class="api-game-card" data-id="${id}">
            ...
            <div class="game-card-content">
                <h2><a href="juego.html?id=${id}" style="color:inherit;text-decoration:none;">${juego.titulo}</a></h2>
...
/**
 * Ver detalles de un juego en un modal
 */
async function verDetallesJuego(id) {
    ...
    const precio = juego.precio != null ? `$${Number(juego.precio).toFixed(2)}` : 'Precio no disponible';
    const imagen = juego.urlImagen || juego.urlimagen || obtenerImagenPorDefecto(juego.titulo);

    document.getElementById('modalImg').src = imagen;
    document.getElementById('modalImg').alt = juego.titulo;
    document.getElementById('modalTitulo').textContent = juego.titulo;
    document.getElementById('modalCalif').textContent = `★ ${calificacion}  ·  ${precio}`;
    document.getElementById('modalDetalles').innerHTML = `
        <span><strong style="color:#e2e8f0;">Estudio:</strong> ${juego.estudio?.nombre || 'N/A'}</span>
        <span><strong style="color:#e2e8f0;">Género:</strong> ${juego.genero?.nombre || 'N/A'}</span>
        <span><strong style="color:#e2e8f0;">Plataforma:</strong> ${juego.plataforma?.nombre || 'N/A'}</span>
        <span><strong style="color:#e2e8f0;">Lanzamiento:</strong> ${juego.fechaLanzamiento || 'N/A'}</span>
    `;
    document.getElementById('modalDesc').textContent = juego.descripcion || '';
